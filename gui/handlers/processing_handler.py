...
class ProcessingHandler:
    ...
    def _initialize_processing(self, selected_clients):
        """Set up organizer and summary data for processing run"""
        ...
        progress_tracker = ProgressTracker(len(selected_clients))

        return timestamp, summary_data, progress_tracker

    def _process_single_client(self, client_key, client_info, idx, total_clients,
                                timestamp, summary_data, progress_tracker):
        """Process folders, files, and reports for one client. Returns (files_count, reports_count, level1_folder)."""
        ...
        for result in file_results:
            if result['status'] == 'Success':
                self.app.root.after(0, lambda: self.app.log_message(
                    f"    📁 {result['filename']} → {result['file_type']} folder", 'normal'))
        ...
        # ITC Report
        itc_reports = self._create_itc_report(client_info, folders, safe_client, safe_state, safe_timestamp, summary_data)
        if itc_reports:
            client_reports += 1
            itc_success = True

        # Sales Report
        sales_reports = self._create_sales_report(client_info, folders, safe_client, safe_state, safe_timestamp, summary_data)
        if sales_reports:
            client_reports += 1
            sales_success = True
        ...
        return client_files, client_reports, level1_folder

    def _create_itc_report(self, client_info, folders, safe_client, safe_state, safe_timestamp, summary_data):
        """Create ITC report for one client. Returns True on success."""
    ...
    def _create_sales_report(self, client_info, folders, safe_client, safe_state, safe_timestamp, summary_data):
        """Create Sales report for one client. Returns True on success."""
        self.app.log_message("  💰 Creating Sales report...", 'info')
        ...
        if not Path(sales_template_path).exists():
    ...
    def _create_summary_report(self, level1_folder, timestamp, summary_data, progress_tracker):
        """Create the Level 1 summary Excel report."""
    ...
    def _show_completion(self, processed, total_clients, total_files, total_reports,
                         level1_folder, start_time, progress_tracker):
        ...
        dur_minutes = int(duration.total_seconds() // 60)
        dur_seconds = int(duration.total_seconds() % 60)
        self.app.log_message(f"\n⏱️ Total processing time: {dur_minutes} minutes {dur_seconds} seconds", 'info')
    ...
    def _cleanup_processing(self):
        """Reset processing state and re-enable buttons."""
    ...
    def process_files_thread(self, selected_clients):
        ...
        try:
            ...
            timestamp, summary_data, progress_tracker = self._initialize_processing(selected_clients)
            ...
            for idx, client_key in enumerate(selected_clients):
                ...
                try:
                    client_files, client_reports, client_level1 = self._process_single_client(
                        client_key, client_info, idx, total_clients,
                        timestamp, summary_data, progress_tracker
                    )
                    ...
                    self.app.log_message(f"  ❌ Failed {client_key}: {str(e)}", 'error')

            # Create summary report
            if processed > 0 and level1_folder:
                self._create_summary_report(level1_folder, timestamp, summary_data, progress_tracker)

            # Show completion
            self._show_completion(
                processed, total_clients, total_files, total_reports,
                level1_folder, start_time, progress_tracker
            )
